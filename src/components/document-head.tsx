...
const DocumentHead = ({ title = '', description = "Keita Uenishi's blog", path = '' }) => {
  const elements = path.split('/')
  const isSlugPath = elements[0] === '' && elements[1] === 'blog' && elements.length === 3
  const isRootPath = path === '' || path === '/'

  const ogImageContent = `${NEXT_PUBLIC_URL}/images/blog-og-image.jpg`
  ...
  return (
    <Head>
      ...
      {NEXT_PUBLIC_URL ? (
        <link rel="canonical" href={new URL(path, NEXT_PUBLIC_URL).toString()} />
      ) : null}
      <meta
        itemProp="name"
        content={title ? `${title} - ${NEXT_PUBLIC_SITE_TITLE}` : NEXT_PUBLIC_SITE_TITLE}
      />
      {ogImageContent ? <meta itemProp="image" content={ogImageContent} /> : null}
      <meta name="description" content={description ? description : NEXT_PUBLIC_SITE_DESCRIPTION} />
      {NEXT_PUBLIC_URL ? (
        <meta property="og:url" content={new URL(path, NEXT_PUBLIC_URL).toString()} />
      ) : null}
      ...
      {ogImageContent ? <meta property="og:image" content={ogImageContent} /> : null}
      <meta name="twitter:card" content="summary_large_image" />
      {ogImageContent ? <meta name="twitter:image" content={ogImageContent} /> : null}
      <meta
        name="twitter:title"
        content={title ? `${title} - ${NEXT_PUBLIC_SITE_TITLE}` : NEXT_PUBLIC_SITE_TITLE}
      />
